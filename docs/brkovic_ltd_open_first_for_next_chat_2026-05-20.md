...
- В этой working copy уже есть коммиты с черновиком multipage/translation schema и collection API; на production они не выкладывались.
...
- Prisma schema сейчас post-centric и RU/EN; моделей multipage/collections/translations пока нет.
- В локальной backend working copy подготовлены модели `JournalCollection`, `JournalCollectionPage`, `JournalTranslation`, `JournalMediaTranslation`, общий `collectionId` для комментариев/лайков/share и SQL-черновик миграции.
- Публичный `/api/public/journal-collections` сейчас возвращает `404`.
- `PublicLikesService` есть, но публичные маршруты like / unlike / like-status не подключены контроллером.
