# BRKOVIC.LTD MVP Task Registry

**Owner:** Director of BRKOVIC.LTD MVP
**Created:** 2026-05-27

## Statuses

- Backlog
- Assigned
- In Progress
- For Review
- Approved
- Blocked
- Gate Closed
- Superseded

## Tasks

| Task ID | Title | Owner Chat | Status | Required Output |
| --- | --- | --- | --- | --- |
| `BRK-MVP-DIR-001` | Establish BRKOVIC.LTD project office | `CHAT-BRK-DIRECTOR-001` | Approved | `director-reports/2026-05-27-office-setup.md` |
| `BRK-MVP-DIR-002` | MVP stabilization order: no interface redesign without approval | `CHAT-BRK-DIRECTOR-001` | Approved | `director-reports/2026-05-27-mvp-stabilization-order.md` |
| `BRK-MVP-DIR-003` | Final local MVP gate review | `CHAT-BRK-DIRECTOR-001` | Approved | `director-reports/2026-05-27-final-mvp-gate-review.md` |
| `BRK-MVP-DIR-004` | Language goals sequential plan | `CHAT-BRK-DIRECTOR-001` | For Review | `director-reports/2026-05-28-language-goals-sequential-plan.md` |
| `BRK-MVP-DIR-005` | Language Sprint 02 launch | `CHAT-BRK-DIRECTOR-001` | For Review | `director-reports/2026-05-28-language-sprint-02-launch.md` |
| `BRK-MVP-DIR-006` | OpenAI Language Desk Sprint 01 launch | `CHAT-BRK-DIRECTOR-001` | For Review | `director-reports/2026-05-28-openai-language-desk-sprint-01-launch.md` |
| `BRK-MVP-DIR-007` | Journal localization unblock sprint | `CHAT-BRK-DIRECTOR-001` | In Progress | `director-reports/2026-05-28-journal-localization-unblock-sprint.md` |
| `BRK-MVP-REL-001` | Role intake and safe release manifest | `CHAT-BRK-RELEASE-001` | For Review | `reports/release-manifest-2026-05-27.md` |
| `BRK-MVP-QAUX-001` | Role intake and local MVP UX smoke | `CHAT-BRK-QA-UX-001` | For Review | `reports/local-qa-ux-smoke-2026-05-27.md` |
| `BRK-MVP-QAUX-002` | NavDesk watch setup visual review | `CHAT-BRK-QA-UX-001` | Ready | `reports/navdesk-watch-setup-visual-review-2026-05-27.md` |
| `BRK-MVP-QAUX-003` | NavDesk tides desktop/tablet/mobile UX QA | `CHAT-BRK-QA-UX-001` | For Review | `reports/navdesk-tides-ux-qa-2026-05-27.md` |
| `BRK-MVP-QAUX-004` | NavDesk tides weekly graph print/PDF UX | `CHAT-BRK-QA-UX-001` | For Review | `reports/navdesk-tides-weekly-graph-print-ux-2026-05-27.md` |
| `BRK-MVP-SEO-001` | Role intake and SEO/I18N audit | `CHAT-BRK-SEO-I18N-001` | For Review | `reports/seo-i18n-audit-2026-05-27.md` |
| `BRK-MVP-LANGAI-001` | AI multilingual journal admin architecture marker | `CHAT-BRK-SEO-I18N-001` owns, `CHAT-BRK-BACKEND-001` consults | For Review | `reports/ai-multilingual-admin-architecture-2026-05-27.md` |
| `BRK-MVP-SEODIR-001` | SEO Integration Director intake and page-agent command | `CHAT-BRK-SEO-DIR-001` | Ready | `reports/seo-integration-director-intake-2026-05-28.md` |
| `BRK-MVP-SEOPAGE-001` | SEO page agent: home | `CHAT-BRK-SEO-DIR-001` | Ready | `cabinets/seo-integration-director/page-agents/page-001-home.md` |
| `BRK-MVP-SEOPAGE-002` | SEO page agent: yacht management | `CHAT-BRK-SEO-DIR-001` | Ready | `cabinets/seo-integration-director/page-agents/page-002-yacht-management.md` |
| `BRK-MVP-SEOPAGE-003` | SEO page agent: IYT training | `CHAT-BRK-SEO-DIR-001` | Ready | `cabinets/seo-integration-director/page-agents/page-003-iyt-training.md` |
| `BRK-MVP-SEOPAGE-004` | SEO page agent: skipper service | `CHAT-BRK-SEO-DIR-001` | Ready | `cabinets/seo-integration-director/page-agents/page-004-skipper-service.md` |
| `BRK-MVP-SEOPAGE-005` | SEO page agent: sailing tours | `CHAT-BRK-SEO-DIR-001` | Ready | `cabinets/seo-integration-director/page-agents/page-005-sailing-tours.md` |
| `BRK-MVP-SEOPAGE-006` | SEO page agent: yacht acceptance and delivery | `CHAT-BRK-SEO-DIR-001` | Ready | `cabinets/seo-integration-director/page-agents/page-006-yacht-acceptance-delivery.md` |
| `BRK-MVP-SEOPAGE-007` | SEO page agent: yacht registration | `CHAT-BRK-SEO-DIR-001` | Ready | `cabinets/seo-integration-director/page-agents/page-007-yacht-registration.md` |
| `BRK-MVP-SEOPAGE-008` | SEO page agent: ship journal | `CHAT-BRK-SEO-DIR-001` | Ready | `cabinets/seo-integration-director/page-agents/page-008-journal.md` |
| `BRK-MVP-SEOPAGE-009` | SEO page agent: NavDesk hub | `CHAT-BRK-SEO-DIR-001` | Ready | `cabinets/seo-integration-director/page-agents/page-009-navdesk-hub.md` |
| `BRK-MVP-SEOPAGE-010` | SEO page agent: NavDesk watch log | `CHAT-BRK-SEO-DIR-001` | Ready | `cabinets/seo-integration-director/page-agents/page-010-navdesk-watch.md` |
| `BRK-MVP-SEOPAGE-011` | SEO page agent: NavDesk tides | `CHAT-BRK-SEO-DIR-001` | Ready | `cabinets/seo-integration-director/page-agents/page-011-navdesk-tides.md` |
| `BRK-MVP-SEOPAGE-012` | SEO page agent: NavDesk route | `CHAT-BRK-SEO-DIR-001` | Ready | `cabinets/seo-integration-director/page-agents/page-012-navdesk-route.md` |
| `BRK-MVP-SEOPAGE-013` | SEO page agent: NavDesk UKV/VHF | `CHAT-BRK-SEO-DIR-001` | Ready | `cabinets/seo-integration-director/page-agents/page-013-navdesk-ukv.md` |
| `BRK-MVP-SEOPAGE-014` | SEO page agent: Maritime English | `CHAT-BRK-SEO-DIR-001` | Ready | `cabinets/seo-integration-director/page-agents/page-014-maritime-english.md` |
| `BRK-MVP-SEOPAGE-015` | SEO page agent: Ship Cashbox | `CHAT-BRK-SEO-DIR-001` | Ready | `cabinets/seo-integration-director/page-agents/page-015-ship-cashbox.md` |
| `BRK-MVP-LOC-001` | Localization architecture and language-surface inventory | `CHAT-BRK-LOC-001` | Ready | `reports/localization-surface-inventory-2026-05-27.md` |
| `BRK-MVP-LOC-002` | NavDesk watch log localization surface follow-up | `CHAT-BRK-LOC-001` | Ready | `reports/navdesk-watch-localization-2026-05-27.md` |
| `BRK-MVP-LOC-003` | Ship Journal translation work order | `CHAT-BRK-LOC-001` | For Review | `reports/journal-localization-work-order-2026-05-28.md` |
| `BRK-MVP-LOC-004` | Target language matrix | `CHAT-BRK-LOC-001` | For Review | `reports/target-language-matrix-2026-05-28.md` |
| `BRK-MVP-LOC-005` | Language interface step 1 | `CHAT-BRK-LOC-001` | For Review | `reports/localization-language-interface-step1-2026-05-28.md` |
| `BRK-MVP-LOC-006` | Page localization readiness step 2 | `CHAT-BRK-LOC-001` | For Review | `reports/localization-page-readiness-step2-2026-05-28.md` |
| `BRK-MVP-LOC-007` | NavDesk maritime glossary | `CHAT-BRK-LOC-001` | For Review | `reports/navdesk-maritime-glossary-2026-05-28.md` |
| `BRK-MVP-LOC-008` | Static-unused key classification | `CHAT-BRK-LOC-001` | For Review | `reports/static-unused-key-classification-2026-05-28.md` |
| `BRK-MVP-LOC-009` | Language Sprint 02 locked/risk copy map | `CHAT-BRK-LOC-001` | For Review | `reports/language-sprint-02-risk-map-2026-05-28.md` |
| `BRK-MVP-LOC-010` | NavDesk protected phrase registry | `CHAT-BRK-LOC-001` | For Review | `reports/navdesk-protected-phrase-registry-2026-05-28.md` |
| `BRK-MVP-LOC-011` | Language Sprint 03: roadmap language execution (DE/IT/ES/SR/ZH) | `CHAT-BRK-LOC-001` | For Review | `reports/language-roadmap-sprint-03-2026-05-29.md` |
| `BRK-MVP-BE-001` | Role intake and backend/admin/API audit | `CHAT-BRK-BACKEND-001` | For Review | `reports/backend-admin-api-audit-2026-05-27.md` |
| `BRK-MVP-BE-002` | OpenAI language desk server boundary | `CHAT-BRK-BACKEND-001` | For Review | `reports/openai-language-desk-implementation-notes-2026-05-27.md` |
| `BRK-MVP-BE-004` | OpenAI live-key health and boundary confirmation | `CHAT-BRK-BACKEND-001` | For Review | `reports/openai-live-key-health-2026-05-28.md` |
| `BRK-MVP-BE-005` | Translation route activation for admin UI | `CHAT-BRK-BACKEND-001` | For Review | `reports/backend-admin-translation-route-activation-2026-05-28.md` |
| `BRK-MVP-BE-007` | Public user auth gateway for protected tools (Google / email code) | `CHAT-BRK-BE-IMPL-001` | Approved | `reports/backend-engineer-tool-auth-gateway-2026-05-29.md` |
| `BRK-MVP-BE-007-A` | Canonical /api/auth/user route + migration aliases deployment order | `CHAT-BRK-BE-IMPL-001` | Ready | `cabinets/backend-engineer/task-0009-be007-canonical-route-deployment.md` |
| `BRK-MVP-BE-010` | Tool auth code delivery and TTL consistency fix | `CHAT-BRK-BE-IMPL-001` | For Review | `cabinets/backend-engineer/task-0010-tool-auth-code-delivery-and-ttl.md` |
| `BRK-MVP-BE-011` | MongoDB Atlas bootstrap and initial PostgreSQL load | `CHAT-BRK-BE-IMPL-001` | For Review | `director-reports/2026-05-31-mongodb-atlas-bootstrap-and-initial-load.md` |
| `BRK-MVP-BE-012` | MongoDB backend public journal bridge | `CHAT-BRK-BE-IMPL-001` | For Review | `director-reports/2026-05-31-mongodb-backend-public-bridge.md` |
| `BRK-MVP-BE-013` | MongoDB backend admin/auth bridge | `CHAT-BRK-BE-IMPL-001` | For Review | `director-reports/2026-05-31-mongodb-backend-admin-auth-bridge.md` |
| `BRK-MVP-BE-014` | MongoDB production cutover package and Atlas network gate | `CHAT-BRK-BE-IMPL-001` | Completed | `director-reports/2026-05-31-mongodb-final-cutover-and-audit.md` |
| `BRK-MVP-BEIMPL-001` | OpenAI language desk backend skeleton plan | `CHAT-BRK-BE-IMPL-001` | For Review | `reports/backend-engineer-openai-language-desk-skeleton-2026-05-27.md` |
| `BRK-MVP-BEIMPL-002` | NavDesk tides place search expansion | `CHAT-BRK-BE-IMPL-001` | For Review | `reports/navdesk-tides-search-expansion-2026-05-27.md` |
| `BRK-MVP-BEIMPL-003` | NavDesk tides weekly graph API | `CHAT-BRK-BE-IMPL-001` | For Review | `reports/navdesk-tides-weekly-graph-api-2026-05-27.md` |
| `BRK-MVP-BEIMPL-004` | OpenAI language desk local backend skeleton implementation | `CHAT-BRK-BE-IMPL-001` | For Review | `reports/backend-engineer-openai-language-desk-local-skeleton-2026-05-28.md` |
| `BRK-MVP-BEIMPL-005` | Enable live journal translation provider (prod-ready mode) | `CHAT-BRK-BE-IMPL-001` | Approved | `reports/backend-engineer-live-journal-translation-provider-2026-05-29.md` |
| `BRK-MVP-BEIMPL-006` | Journal translation production hardening (provider health/idempotency/validation) | `CHAT-BRK-BE-IMPL-001` | Approved | `reports/backend-engineer-journal-translation-production-hardening-2026-05-29.md` |
| `BRK-MVP-FE-001` | Frontend/NavDesk engineer role intake | `CHAT-BRK-FE-IMPL-001` | For Review | `reports/frontend-engineer-role-intake-2026-05-27.md` |
| `BRK-MVP-FE-002` | NavDesk tides autocomplete frontend implementation | `CHAT-BRK-FE-IMPL-001` | For Review | `reports/navdesk-tides-frontend-autocomplete-2026-05-27.md` |
| `BRK-MVP-FE-003` | NavDesk tides weekly graph and print frontend | `CHAT-BRK-FE-IMPL-001` | For Review | `reports/navdesk-tides-weekly-graph-frontend-2026-05-27.md` |
| `BRK-MVP-FE-004` | NavDesk stability hotfix review | `CHAT-BRK-FE-IMPL-001` | Assigned | `reports/navdesk-frontend-stability-hotfix-review-2026-05-27.md` |
| `BRK-MVP-FE-005` | NavDesk frontend polish sprint: UKV night cards and main page hierarchy | `CHAT-BRK-FE-IMPL-001` | For Review | `reports/navdesk-frontend-polish-sprint-2026-05-27.md` |
| `BRK-MVP-FE-006` | NavDesk route print/PDF frontend implementation | `CHAT-BRK-FE-IMPL-001` | For Review | `reports/navdesk-route-print-frontend-2026-05-27.md` |
| `BRK-MVP-FE-007` | NavDesk main card-order acceptance | `CHAT-BRK-FE-IMPL-001` | Approved | `reports/navdesk-main-card-order-acceptance-2026-05-28.md` |
| `BRK-MVP-FE-008` | Language menu model fix for many-language readiness | `CHAT-BRK-FE-IMPL-001` | For Review | `reports/frontend-language-menu-model-fix-2026-05-28.md` |
| `BRK-MVP-FE-009` | Language panel polish | `CHAT-BRK-FE-IMPL-001` | For Review | `reports/frontend-language-panel-polish-2026-05-28.md` |
| `BRK-MVP-FE-010` | Language roadmap panel | `CHAT-BRK-FE-IMPL-001` | For Review | `reports/frontend-language-roadmap-panel-2026-05-28.md` |
| `BRK-MVP-FE-011` | Language roadmap interface step 1 | `CHAT-BRK-FE-IMPL-001` | For Review | `reports/frontend-language-roadmap-interface-step1-2026-05-28.md` |
| `BRK-MVP-FE-012` | I18N surface step 2 | `CHAT-BRK-FE-IMPL-001` | For Review | `reports/frontend-i18n-surface-step2-2026-05-28.md` |
| `BRK-MVP-FE-013` | I18N attribute support | `CHAT-BRK-FE-IMPL-001` | For Review | `reports/frontend-i18n-attribute-support-2026-05-28.md` |
| `BRK-MVP-FE-014` | Accessibility attribute keys | `CHAT-BRK-FE-IMPL-001` | For Review | `reports/frontend-accessibility-attribute-keys-2026-05-28.md` |
| `BRK-MVP-FE-015` | I18N diagnostics gate | `CHAT-BRK-FE-IMPL-001` | For Review | `reports/i18n-diagnostics-2026-05-28.md` |
| `BRK-MVP-FE-016` | Safe JS UI i18n first migration | `CHAT-BRK-FE-IMPL-001` | For Review | `reports/frontend-safe-js-ui-i18n-first-migration-2026-05-28.md` |
| `BRK-MVP-FE-017` | Journal safe JS UI i18n layer | `CHAT-BRK-FE-IMPL-001` | For Review | `reports/frontend-journal-safe-js-ui-i18n-layer-2026-05-28.md` |
| `BRK-MVP-FE-018` | Delivery calculator safe JS UI i18n layer | `CHAT-BRK-FE-IMPL-001` | For Review | `reports/frontend-delivery-safe-js-ui-i18n-layer-2026-05-28.md` |
| `BRK-MVP-FE-019` | NavDesk small calculator i18n alias | `CHAT-BRK-FE-IMPL-001` | For Review | `reports/frontend-navdesk-small-calculator-i18n-alias-2026-05-28.md` |
| `BRK-MVP-FE-020` | Language menu modal window and per-language variants | `CHAT-BRK-FE-IMPL-001` | For Review | `reports/frontend-language-menu-modal-window-2026-05-29.md` |
| `BRK-MVP-FE-021` | Tool access gating flow (Google + 6-digit code) | `CHAT-BRK-FE-IMPL-001` | For Review | `reports/frontend-tool-auth-gating-flow-2026-05-29.md` |
| `BRK-MVP-FE-022` | Vetus Nauta PWA install reminder flow | `CHAT-BRK-FE-IMPL-001` | Approved | `reports/frontend-pwa-install-reminder-flow-2026-05-30.md` |
| `BRK-MVP-QAUX-005` | NavDesk functional and print/PDF audit | `CHAT-BRK-QA-UX-001` | Assigned | `reports/navdesk-functional-print-audit-2026-05-27.md` |
| `BRK-MVP-QAUX-006` | Language menu and system-language hint QA | `CHAT-BRK-QA-UX-001` | Superseded | `reports/language-menu-qa-2026-05-28.md` |
| `BRK-MVP-QAUX-012` | Language menu and system-language hint re-check after modal rollout | `CHAT-BRK-QA-UX-001` | In Progress | `reports/language-menu-qa-2026-05-29.md` |
| `BRK-MVP-QAUX-007` | NavDesk language layer smoke | `CHAT-BRK-QA-UX-001` | For Review | `reports/navdesk-language-layer-smoke-2026-05-28.md` |
| `BRK-MVP-QAUX-008` | Language interactive smoke | `CHAT-BRK-QA-UX-001` | For Review | `reports/language-interactive-smoke-2026-05-28.md` |
| `BRK-MVP-QAUX-009` | Language Sprint 02 regression smoke | `CHAT-BRK-QA-UX-001` | For Review | `reports/language-sprint-02-regression-smoke-2026-05-28.md` |
| `BRK-MVP-QAUX-010` | Language Sprint 02 delivery smoke | `CHAT-BRK-QA-UX-001` | For Review | `reports/language-sprint-02-delivery-smoke-2026-05-28.md` |
| `BRK-MVP-QAUX-011` | NavDesk small calculator i18n smoke | `CHAT-BRK-QA-UX-001` | For Review | `reports/navdesk-small-calculator-i18n-smoke-2026-05-28.md` |
| `BRK-MVP-QAUX-013` | Tool auth gate smoke (desktop/tablet/mobile) | `CHAT-BRK-QA-UX-001` | In Review | `reports/qa-tool-auth-gate-smoke-2026-05-29.md` |
| `BRK-MVP-QAUX-014` | Tool auth OTP re-check after BE-010 | `CHAT-BRK-QA-UX-001` | Ready After Deploy | `reports/backend-tool-auth-otp-gate-smoke-2026-05-30.md` |
| `BRK-MVP-QAUX-015` | PWA install reminder smoke | `CHAT-BRK-QA-UX-001` | Approved | `reports/qa-pwa-install-reminder-smoke-2026-05-30.md` |
| `BRK-MVP-BE-003` | NavDesk tides function audit | `CHAT-BRK-BACKEND-001` | For Review | `reports/navdesk-tides-audit-2026-05-27.md` |
| `BRK-MVP-DEPLOY-001` | Controlled production deploy | `CHAT-BRK-DEPLOY-001` | Gate Closed | `reports/deploy-report-2026-05-27.md` |
| `BRK-MVP-PRODQA-001` | Production smoke and indexing QA | `CHAT-BRK-PROD-QA-001` | Gate Closed | `reports/production-smoke-seo-indexing-2026-05-27.md` |

## Immediate Next Step

Run these active streams in order:

1. Keep PostgreSQL as rollback source during the first Mongo cutover window.
2. Run post-cutover QA for frontend/API contract alignment, especially live images, menu buttons, NavDesk tool pages and admin-management entry.
3. Separate cosmetic/live frontend fixes from database migration work.
4. Schedule a second-day Mongo health and admin write-path check after real usage.

Owner chats to keep in sync:

1. `CHAT-BRK-BE-IMPL-001`
2. `CHAT-BRK-FE-IMPL-001`
3. `CHAT-BRK-QA-UX-001`
4. `CHAT-BRK-DIRECTOR-001` (for final decision lock)
