# BRKOVIC.LTD Office Reports

Worker chats write full reports here.

Do not store secrets, credentials, tokens, private configs, cookies, sessions, or FTP details in this folder.

Expected first reports:

- `release-manifest-2026-05-27.md`
- `local-qa-ux-smoke-2026-05-27.md`
- `seo-i18n-audit-2026-05-27.md`
- `ai-multilingual-admin-architecture-2026-05-27.md`
- `openai-api-key-server-boundary-2026-05-27.md`
- `openai-language-desk-implementation-notes-2026-05-27.md`
- `backend-engineer-openai-language-desk-skeleton-2026-05-27.md`
- `openai-live-key-health-2026-05-28.md`
- `backend-engineer-openai-language-desk-local-skeleton-2026-05-28.md`
- `navdesk-tides-audit-2026-05-27.md`
- `localization-surface-inventory-2026-05-27.md`
- `backend-admin-api-audit-2026-05-27.md`
- `deploy-report-2026-05-27.md`
- `production-smoke-seo-indexing-2026-05-27.md`
