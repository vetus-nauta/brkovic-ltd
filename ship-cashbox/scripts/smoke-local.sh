...
SHELL_VERSION="${SHELL_VERSION:-20260603-cashbox-entry-final-order-01}"
API_VERSION="${API_VERSION:-2026.06.03-ship-cashbox-local-contract-01}"
SW_CACHE="${SW_CACHE:-ship-cashbox-shell-v20260603-105}"
...
contains html_workspace_start "workspaceStartButton" "$TMP_DIR/index.html"
...
if grep -q 'api("boot")\|api(`boot`' "$TMP_DIR/app.js"; then
  echo "modeless_boot FAIL" >&2
  exit 1
fi
...
for mode in group personal; do
  ...
  node -e "const fs=require('fs'); const mode=process.argv[2]; const data=JSON.parse(fs.readFileSync(process.argv[1],'utf8')); if (data.session && data.session.session_mode !== mode) { console.error('expected '+mode+', got '+data.session.session_mode); process.exit(1); }" "$out" "$mode" \
    ...
    || exit 1
  node -e "const fs=require('fs'); const mode=process.argv[2]; const data=JSON.parse(fs.readFileSync(process.argv[1],'utf8')); const leak=(data.archive||[]).find((item)=>item.session_mode!==mode); if (leak) { console.error('archive leak for '+mode+': '+leak.session_mode+' '+leak.id); process.exit(1); }" "$out" "$mode" \
